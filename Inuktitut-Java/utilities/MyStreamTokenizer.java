...
// -----------------------------------------------------------------------
// Document/File:		MyStreamTokenizer.java
//
...
//
// Description: Version particulière de StreamTokenizer.
//
// -----------------------------------------------------------------------
...
// $Id: MyStreamTokenizer.java,v 1.1 2009/06/19 19:38:33 farleyb Exp $
//
// Commentaires RCS---------------------------------------RCS Log Messages
//
// $Log: MyStreamTokenizer.java,v $
...
// Initial revision
//
// Revision 1.0  2002-12-03 12:37:01-05  farleyb
// Initial revision
//
//
// -------------------\\Information RCS Information//---------------------
...
public class MyStreamTokenizer {
    ...
    private char spacebuf[] = new char[20];  /* BF */
    private char nbbuf[] = new char[20];     /* BF */
    ...
    public String spaceval;    /* BF */
    public String nbval;       /* BF */
	...
	quoteChar('\'');
	parseNumbers();
	wordChars(5120,5759);  /* inuktitut unicode */
    }
	...
	spaceval = String.copyValueOf(spacebuf, 0, 0);   /* BF */
	if (pushedBack) {
	    pushedBack = false;
	    return ttype;
	}
	...
	while ((ctype & CT_WHITESPACE) != 0) {
	    //< BF
	    if (j >= spacebuf.length) {
		char spacenb[] = new char[spacebuf.length * 2];
		System.arraycopy(spacebuf, 0, spacenb, 0, spacebuf.length);
		spacebuf = spacenb;
	    }
	    spacebuf[j++] = (char) c;
	    //> BF
		...
		if (eolIsSignificantP) {
		    peekc = SKIP_LF;
		    spaceval = String.copyValueOf(spacebuf, 0, j);  /* BF */
		    return ttype = TT_EOL;
		}
		c = read();
		if (c == '\n')
	...
	if ((ctype & CT_DIGIT) != 0) {
		...
		//< BF
		if (k >= nbbuf.length) {
		    char nbnb[] = new char[nbbuf.length * 2];
		    System.arraycopy(nbbuf, 0, nbnb, 0, nbbuf.length);
		    nbbuf = nbnb;
		}
		nbbuf[k++] = (char)c;
		//> BF
	    ...
	    nval = neg ? -v : v;
	    nbval = String.copyValueOf(nbbuf, 0, k);   /* BF */
